...
export default function Page() {
  ...
  const [alertError, alertWarning] = useAlert((state: any) => [state.error, state.warning]);
    ...
    <main className="flex flex-col items-center _justify-between _p-24">
      ...
      <div className="flex flex-col lg:flex-row lg:space-x-4 items-center justify-center mt-4">
        <div className="text-dark-2">
          <Link
            href="/"
            onClick={(e) => method == "login-email" ? doLogin(e, signin, form, router, alertWarning) : method == "signup-email" ? doSignup(e, signin, form, router, alertWarning) : alertError(`Unknown signing method: ${method}`)}
          >
            {method == "login-email" ? "Login" : method == "signup-email" ? "Signup" : "(unknown method)"}
          </Link>
        </div>
